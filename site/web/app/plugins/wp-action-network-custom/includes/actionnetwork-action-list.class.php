...
class Actionnetwork_Action_List extends WP_List_Table {
	...
	function column_shortcode($item) {
		$shortcode = "[actionnetwork id={$item['id']}]";
		$__copy = __( 'Copy', 'actionnetwork' );
		$__options = __( 'Options', 'actionnetwork' );
		$shortcode_html = <<<EOHTML
<div class="copy-wrapper">
<input type="text" class="copy-text" readonly="readonly" id="shortcode-{$item['id']}" value="$shortcode" /><button data-copytarget="#shortcode-{$item['id']}" class="copy">$__copy</button>
</div>
EOHTML;
		if ($item['an_id']) {
	...
	function column_meta($item) {
		...
		if ($source != 'API' && (($item['type'] == 'event') || ($item['type'] == 'ticketed_event'))) {
			$info .= '<div><a href="';
			$info .= wp_nonce_url(
				admin_url('admin.php?page=actionnetwork&actionnetwork_admin_action=edit_event&actionnetwork_event_id='.$item['id']),
    ...
    function process_bulk_action() {
        ...
        if( 'delete'===$this->current_action() ) {
			...
			foreach ( $delete_ids as $id ) {
				if ($wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}actionnetwork` WHERE an_id='' AND id=$id")) {
        ...
        if( 'hide'===$this->current_action() ) {
			$hide_ids = esc_sql( $_REQUEST['bulk-action'] );
        ...
        if( 'unhide'===$this->current_action() ) {
			$unhide_ids = esc_sql( $_REQUEST['bulk-action'] );
			$cannot_unhide_synced_disabled_actions = 0;
			$unhidden_actions = 0;
			foreach ( $unhide_ids as $id ) {
				if ($wpdb->get_var("SELECT COUNT(*) FROM `{$wpdb->prefix}actionnetwork` WHERE an_id != '' AND enabled=0 AND id=$id")) {
					$cannot_unhide_synced_disabled_actions++;
				} else {
					self::unhide_action( $id );
					$unhidden_actions++;
				}
			}
